...
export const SoundverseRoom: FC<RoomProps> = ({ room }) => {
  ...
  useEffect(() => {
    const newAnonArray = new Array(room?.currentAnonymousUsers > 0 ? room.currentAnonymousUsers : 0)
    newAnonArray.fill(0)
  ...
  return (
    <div className="flex justify-center relative overflow-hidden h-[85vh] flex-wrap lg:flex-nowrap">
      ...
      <div className="flex flex-col sm:ml-10 w-full xl:mr-[360px] text-sm h-screen">
        <div
          className={cn(
      ...
      <div className="absolute right-[310px]">
        <Chat roomId={room.id} chat={room.chat} className="-mt-1"/>
      </div>
    </div>
  )
}
